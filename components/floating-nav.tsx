...
export function FloatingNav() {
  ...
  return (
    <>
      ...
      >
        <div className="relative px-4 py-3 rounded-full bg-[#212121]/80 backdrop-blur-md border border-zinc-700/50 shadow-lg shadow-[#0D7377]/10 hover:shadow-[#0D7377]/20 transition-all duration-300">
          <div className="absolute -inset-0.5 bg-gradient-to-r from-[#0D7377]/20 to-[#32E0C4]/20 rounded-full blur opacity-50"></div>
          ...
          <div className="flex flex-col items-center justify-center h-full">
            {navItems.map((item) => (
              <Link
                key={item.name}
                href={item.href}
                className="px-8 py-4 text-2xl font-medium text-white hover:text-[#32E0C4] transition-colors"
                onClick={handleNavClick}
              >
                {item.name}
              </Link>
